...
class BiMLMPredictor:
    ...
    def _load_model(self) -> torch.nn.Module:
        ...
        try:
            ...
            raise Exception("模型文件不存在，请先训练模型")
    ...
    def predict_next(self, user_features: Dict) -> List[int]:
        ...
        with torch.no_grad():
            ...
            # 获取预测分数
            scores, _ = self.model(sequence_tensor, ratings_tensor, self.audio_embeddings, training=False)
    ...
    def batch_predict(self, user_features_list: Dict[int, Dict]) -> Dict[int, List[int]]:
        ...
        for i in range(0, len(user_ids), batch_size):
            ...
            # 批量预测
            with torch.no_grad():
                scores, _ = self.model(batch_sequences, batch_ratings_tensor, self.audio_embeddings, training=False)
                ...
                for j, user_id in enumerate(batch_user_ids):
                    ...
                    for song_id, rating in user_ratings.items():
                        if song_id < len(user_scores):
                            user_scores[song_id] *= (1 + float(rating))
