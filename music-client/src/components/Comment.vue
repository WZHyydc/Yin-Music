<template>
  ...
  <ul class="popular">
    <li v-for="(item, index) in commentList" :key="index">
      ...
      <div ref="up"  @click="setSupport(item.id, item.up, userId)">
...
async function getComment(id) {
  try {
    ...
    for (let index = 0; index < commentList.value.length; index++) {
      // 获取评论用户的昵称和头像
      // console.log(commentList.value[index].userId);
      ...
      // console.log(resultUser.data[0].avator);
      commentList.value[index].username = resultUser.data[0].username;
      // console.log(resultUser.data[0].username);
    }
  } catch (error) {
